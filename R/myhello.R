#' Hello World
#'
#' `myhello` says _"Hello"_ in the user-specified language. The user is asked to give her/his name so that the hello message gets personalized.
#'
#' @return `"Hello, world!"`
#' @param
...
#'
#' @return
#' a `caracter` vector with a personalized _"hello"_ message.
#'
#' @export

myhello <- function(who, lang, LangData) {
  if (!exists("who", mode="caracter")|length(who)>1){
  ...
  ifelse(
    ...
    stringr::str_c("Sorry, ", who, ", ", "your language ", "('", lang, "') ", "is not available!", sep = "") |> cat())
